...
let addcategory = async (body: { name: string }) => {
  try {
    ...
    let res = await fetch(`${apiUrl}/categories/create`, {
      method: "POST",
      headers: {
...
const Createcategory = () => {
  ...
  return (
    <main>
      ...
      >
        {({ isSubmitting }) => (
          <Form className="max-w-[500px] relative left-1/2 -translate-x-1/2">
            ...
            <button
              className="bg-primary text-white p-2 px-4 text-sm rounded-lg relative left-1/2 -translate-x-1/2"
              type="submit"
